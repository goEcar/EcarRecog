...
public class ImageUtils {
    ...
    //视频流转为rgb数组(3数组个为一组为一个rgb )
	public static byte[] YUV420SPToRBG(byte[] yuv420sp, int width,
									   int height) {
		Bitmap bitmap = GetBitmapFromYUV420SP(yuv420sp, width, height);
		return getRGB(bitmap);
	}
	static public byte[] getRGB(Bitmap bitmap) {
		int nWidth = bitmap.getWidth();
		int nHeight = bitmap.getHeight();

		int[] pixels = new int[nWidth * nHeight];
		bitmap.getPixels(pixels, 0, nWidth, 0, 0, nWidth, nHeight);

		byte[] pic = new byte[nWidth * nHeight * 3];
		int j = 0;

		/*for (int m = 0; m < nHeight * nWidth; m++) {
			int k = pixels[m];
	...
	public static boolean SaveBitmapToPNG(Bitmap bitmap, File pictureFile) {
		...
		if (!pictureFile.exists()) {
			try {
				FileOutputStream out = new FileOutputStream(pictureFile);
				if (mBitmap.compress(Bitmap.CompressFormat.PNG, 80, out)) {
					out.flush();
					out.close();
				}
			} catch (IOException e) {
	...
	private static Bitmap compressImage(Bitmap image) {
		...
		image.compress(Bitmap.CompressFormat.JPEG, 15, baos);// 质量压缩方法，这里100表示不压缩，把压缩后的数据存放到baos中
		int options = 100;
